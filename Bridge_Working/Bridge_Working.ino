...
void gatesInit() {      //   to  ROAD first time around
  if (gatecheck > 0){
    ...
    myservo2.write (135, 5, true);   //   to across track ?
    myservo2.detach();
...
void   gates2rail() {
  ...
  //  Move gates
    ...
    myservo1.write(135, 5, true);   //  close
    myservo1.detach();
//  second gate
    myservo2.attach(9);
    myservo2.write(135, 5, true);  // close 
    myservo2.detach();
    gatepos = 1;
    DCmotor1.setSpeed(255);   // max glim     
...
void  bridgeup() {
   ...
   if (bridgebutton < 500) {      //   bridge button -  lift  bridge
    ...
    DCmotor1.run(BACKWARD);    // Signal RED
      beep = 0;
      while ( beep < 20) {       //   ten oranges before moving
      digitalWrite(2, HIGH);
      delay(20);
      digitalWrite(2, LOW);
      delay(500);
...
void gates2road() {
  gatebutton = analogRead(A4);//  IS GATEBUTTON PRESSED FOR ROAD?
    ...
    myservo2.write(35, 5, true);   //  close
    myservo2.detach();

    myservo1.attach(10);             // first servo on pin 9
    myservo1.write(35, 5, true);   // close 
    myservo1.detach();
//  gates moved
