//  PWM train controller with parabolic tweak to regulator curve in simple

//  CaderIfor by Michael Beer
//  24/06/21  WORKING   :-)

//  12/02/22    Switchable parabolic from back panel switch
//  Compiles

//   22/08/21   Tweak to simple control regulator curve
//   on regulator in simple and on motorValue in complicated
//   parabolic switchinable to give fast rise at low values, tapering off
...
//   28/05/21   EStop proc done - compiles
...
bool Parabolic = LOW;    //  mode select
...
void setup() {
  ...
  pinMode(5, INPUT);    //   SIMPLE switch and Parabolic switch
  ...
  if (FwdRev < 20) {
    ...
    digitalWrite(6 , LOW);
     ...
     regulator = (regulator / 5);    //   max < 255
      Parabolic = digitalRead(5);    //   read mode switch
        if (Parabolic = LOW) { 
          regulator = 15.9*(sqrt(regulator));
         }
...
void motorDrive() {
  if (esLatch == HIGH ) { // if no probs
   ...
   Parabolic = digitalRead(5);    //   read mode switch
   if (Parabolic = LOW) {              
      motorValue = 15.9*(sqrt(motorValue));  //  slope tweak
   }
